...
private:
  ...
  nsRefPtr<OSXSpeechSynthesizerService> mSpeechService;
};
                                   ...
                                   nsISpeechTask* aTask)
{
  ...
  nsRefPtr<SpeechTaskCallback> callback = new SpeechTaskCallback(aTask, synth);
                                     ...
                                     const char16_t* aData)
{
  if (!strcmp(aTopic, "profile-after-change")) {
    DebugOnly<bool> ret = Init();
    NS_ASSERTION(ret, "Cannot initialize Speech service");
  }
  return NS_OK;
}
...
{
  nsRefPtr<OSXSpeechSynthesizerService> speechService = GetInstance();
